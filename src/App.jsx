...
const DEFAULT_TASKS = [
  {id:"t1", time:"04:00",emoji:"🌅",title:"Réveil",               sub:"Verre d'eau + prière 5 min",       category:"spirituel", freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t2", time:"04:15",emoji:"💪",title:"Sport maison",         sub:"Pompes · Squats · Gainage",        category:"sport",     freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t3", time:"04:45",emoji:"🚿",title:"Douche + Petit-déj",   sub:"Bien manger pour bien travailler", category:"routine",   freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t4", time:"05:15",emoji:"📚",title:"Cours Terminale",      sub:"Concours · Pomodoro 25/5",         category:"etudes",    freq:"weekdays", days:[1,2,3,4,5]},
  {id:"t5", time:"07:15",emoji:"💻",title:"Programmation Web",    sub:"freeCodeCamp · The Odin Project",  category:"etudes",    freq:"weekdays", days:[1,2,3,4,5]},
  {id:"t6", time:"09:15",emoji:"💑",title:"Pause — Message",      sub:"Rester connecté",                  category:"relation",  freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t7", time:"09:30",emoji:"📚",title:"Révision ou Coding",   sub:"Pratique sur projets concrets",    category:"etudes",    freq:"weekdays", days:[1,2,3,4,5]},
  {id:"t8", time:"12:00",emoji:"🍽️",title:"Déjeuner + Repos",     sub:"Recharge complète",                category:"routine",   freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t9", time:"13:00",emoji:"😴",title:"Sieste courte",        sub:"Maximum 20 minutes",               category:"routine",   freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t10",time:"15:30",emoji:"🎌",title:"Animés & Séries",      sub:"Ta récompense — 2h libres",        category:"repos",     freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t11",time:"17:30",emoji:"🚶",title:"Sortie dehors",        sub:"Marche 20 min",                    category:"sport",     freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t12",time:"18:00",emoji:"📚",title:"Dernière session",     sub:"Consolider la journée",            category:"etudes",    freq:"weekdays", days:[1,2,3,4,5]},
  {id:"t13",time:"19:30",emoji:"🍽️",title:"Dîner",               sub:"Bien manger",                      category:"routine",   freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t14",time:"21:00",emoji:"📵",title:"BlockSite activé",     sub:"Cloudflare Family DNS",            category:"protection",freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t15",time:"21:15",emoji:"💑",title:"Appel du soir",        sub:"Connexion émotionnelle",           category:"relation",  freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t16",time:"21:45",emoji:"🙏",title:"Prière du soir",       sub:"Honnête et simple avec Dieu",     category:"spirituel", freq:"daily",    days:[0,1,2,3,4,5,6]},
  {id:"t17",time:"22:00",emoji:"😴",title:"Dors en paix",         sub:"Téléphone loin du lit",            category:"routine",   freq:"daily",    days:[0,1,2,3,4,5,6]},
];
...
function getStrength(tid,ap){
  let h=0;
  for(let i=0;i<14;i++){ const d=new Date(); d.setDate(d.getDate()-i); if(ap[dStr(d)]?.[tid]) h++; }
  const s=Math.round((h/14)*100);
  if(s===0) return{s,label:"Nouvelle",    color:"#444"};
  if(s<30)  return{s,label:"Fragile",     color:"#ef5350"};
  if(s<60)  return{s,label:"En croissance",color:"#FFD740"};
  if(s<85)  return{s,label:"Solide",      color:"#66BB6A"};
  return          {s,label:"Imbattable",  color:"#00E676"};
}
  ...
  useEffect(()=>{
    if(run){
      ref.current=setInterval(()=>setSecs(s=>{
        if(s<=1){ clearInterval(ref.current); setRun(false); const nw=!isWork; setIsWork(nw); if(!nw)setCycles(c=>c+1); return nw?WORK:BRK; }
        return s-1;
      }),1000);
  ...
  return <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:400,backdropFilter:"blur(8px)"}}>
    <div onClick={e=>e.stopPropagation()} className="abc" style={{background:"#0d0d0d",border:"1px solid #1e1e1e",borderRadius:24,padding:"32px 24px",width:"85%",maxWidth:340,textAlign:"center",fontFamily:ff}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:14}}>
        <button className="bm" onClick={()=>setRun(r=>!r)} style={{flex:1,background:accent,color:"#000",border:"none",borderRadius:12,padding:"12px 0",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:ff}}>
          {run?"⏸ Pause":"▶ Démarrer"}
        </button>
...
function ChainCalendar({ap,tasks,accent}){
  ...
  let streak=0,ls=0,cs=0;
  for(let i=59;i>=0;i--){ const d=new Date(); d.setDate(d.getDate()-i); const ds=dStr(d); const tt=tasks.filter(t=>isForDay(t,d)); const dc=ap[ds]?Object.keys(ap[ds]).length:0; const p=tt.length?dc/tt.length:0; if(p>=0.5){cs++;ls=Math.max(ls,cs);}else cs=0; }
  cs=0; for(let i=0;i<60;i++){ const d=new Date(); d.setDate(d.getDate()-i); const ds=dStr(d); const tt=tasks.filter(t=>isForDay(t,d)); const dc=ap[ds]?Object.keys(ap[ds]).length:0; const p=tt.length?dc/tt.length:0; if(p>=0.5){cs++;}else{if(i>0)break;} } streak=cs;
  const days=[]; for(let i=0;i<fdow;i++)days.push(null); for(let d=1;d<=dim;d++)days.push(d);
  const getCol=(day)=>{
    if(!day) return "transparent";
    const d=new Date(y,m,day); if(d>now) return "#0d0d0d";
    const ds=dStr(d); const dc=ap[ds]?Object.keys(ap[ds]).length:0; const tt=tasks.filter(t=>isForDay(t,d));
    if(!tt.length) return "#0d0d0d"; const p=dc/tt.length;
    if(p===0) return "#1a1a1a"; if(p<0.5) return "#2d1a00"; if(p<0.8) return accent+"66"; return accent;
  };
  return <Card>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <ST c={accent}>🔥 Chaîne de discipline</ST>
      <div style={{fontSize:11,color:"#555"}}>{MONTHS_FR[m]} {y}</div>
    </div>
    <div style={{display:"flex",justifyContent:"space-around",marginBottom:16}}>
      {[["Streak actuel",streak+"j",accent],["Meilleur",ls+"j","#66BB6A"]].map(([lbl,val,c])=>(
        <div key={lbl} style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:"#444"}}>{lbl}</div>
        </div>
      ))}
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
      {days.map((day,i)=>{
        const isT=day===now.getDate()&&m===now.getMonth();
        return <div key={i} className={day?"cd":""} style={{aspectRatio:"1",borderRadius:6,background:getCol(day),border:isT?`2px solid ${accent}`:"1px solid #0f0f0f",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:day?(isT?accent:"#333"):"transparent",fontWeight:isT?800:400}}>{day||""}</div>;
      })}
    </div>
    <div style={{display:"flex",gap:10,marginTop:12,fontSize:9,color:"#444",justifyContent:"center"}}>
      {[["#1a1a1a","0%"],[accent+"66","50-79%"],[accent,"80%+"]].map(([c,l])=>(
        <div key={l} style={{display:"flex",alignItems:"center",gap:3}}><div style={{width:9,height:9,borderRadius:2,background:c}}/><span>{l}</span></div>
      ))}
    </div>
  </Card>;
}
  ...
  return <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:300,backdropFilter:"blur(6px)"}}>
    <div className="amd" onClick={e=>e.stopPropagation()} style={{background:"#0d0d0d",border:"1px solid #1e1e1e",borderRadius:"22px 22px 0 0",padding:"8px 20px 52px",width:"100%",maxWidth:440,maxHeight:"90vh",overflowY:"auto",fontFamily:ff,color:"#ebebeb"}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16}}>
        {EMOJIS.map(e=><button key={e} onClick={()=>f("emoji",e)} style={{background:form.emoji===e?"#1c1500":"#0a0a0a",border:`1.5px solid ${form.emoji===e?"var(--acc)":"#1e1e1e"}`,borderRadius:9,padding:"6px 8px",fontSize:18,cursor:"pointer",transition:"all .18s"}}>{e}</button>)}
      </div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16}}>
        {CATEGORIES.map(c=><button key={c} onClick={()=>f("category",c)} style={{background:form.category===c?`${CAT_COLORS[c]}18`:"transparent",border:`1.5px solid ${form.category===c?CAT_COLORS[c]:"#1e1e1e"}`,borderRadius:20,padding:"5px 13px",fontSize:12,fontWeight:form.category===c?700:400,cursor:"pointer",color:form.category===c?CAT_COLORS[c]:"#555",fontFamily:ff,transition:"all .18s"}}>{CAT_ICONS[c]} {c}</button>)}
      </div>
      <div style={{fontSize:11,color:"#555",letterSpacing:1.5,textTransform:"uppercase",marginBottom:8}}>📅 Fréquence</div>
      <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:16}}>
        {FREQ_OPTS.map(opt=><button key={opt.id} onClick={()=>selFreq(opt)} style={{background:form.freq===opt.id?"#1c1500":"transparent",border:`1.5px solid ${form.freq===opt.id?"var(--acc)":"#1e1e1e"}`,borderRadius:12,padding:"10px 14px",fontSize:13,cursor:"pointer",color:form.freq===opt.id?"var(--acc)":"#666",fontFamily:ff,textAlign:"left",fontWeight:form.freq===opt.id?700:400,display:"flex",justifyContent:"space-between",transition:"all .18s"}}><span>{opt.label}</span>{form.freq===opt.id&&<span>✓</span>}</button>)}
      </div>
      {form.freq==="custom"&&<div style={{marginBottom:16}}>
        <div style={{fontSize:11,color:"#555",marginBottom:8}}>Sélectionne les jours :</div>
        <div style={{display:"flex",gap:6,justifyContent:"center"}}>
          {DAYS_SHORT.map((d,i)=><button key={i} onClick={()=>toggleDay(i)} style={{width:36,height:36,borderRadius:18,background:form.days.includes(i)?"var(--acc)":"#0a0a0a",border:`1.5px solid ${form.days.includes(i)?"var(--acc)":"#2a2a2a"}`,color:form.days.includes(i)?"#000":"#555",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:ff,transition:"all .22s"}}>{d}</button>)}
        </div>
      </div>}
...
function AuthScreen({onLogin}){
  ...
  return <div className="afd" style={{background:"#080808",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:ff,color:"#ebebeb",maxWidth:440,margin:"0 auto"}}>
    ...
    <div className="aup" style={{background:"#0f0f0f",border:"1px solid #191919",borderRadius:24,padding:24,width:"100%",maxWidth:360,boxShadow:"0 24px 80px rgba(0,0,0,.6)"}}>
      ...
      <div style={{fontSize:11,color:"#333",textAlign:"center",marginTop:12,lineHeight:1.7}}>
        🔒 Firebase disponible après déploiement sur Vercel
      </div>
    </div>
...
export default function App(){
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{ setQVis(false); setTimeout(()=>{ setQIdx(i=>(i+1)%QUOTES.length); setQVis(true); },600); },8000);
  ...
  const resetToday=async()=>{ const nap={...allProg};delete nap[ts]; setCompleted({});setAllProg(nap);await sSet(`prog_${username}`,nap); setConfirm(null); addToast("Progression d'aujourd'hui réinitialisée","🔄","#0d1a0d","#1a3a1a","#81C784"); };
  ...
  const getAI=async()=>{
    ...
    try{
      const sched=tasks.map(t=>`${t.time} ${t.title} [${t.category}] (${t.freq})`).join("\n");
      const xp=getXP(allProg,tasks),lv=getLevel(xp);
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",
        headers:{"Content-Type":"application/json","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:`Tu es APEX, une IA spécialisée en Data Science comportementale et neuroscience de la productivité. Étudiant 19 ans (concours + dev web + sport + foi). Niveau ${lv.level} - ${lv.name} (${xp} XP).\nProgramme:\n${sched}\nDonne 4 recommandations numérotées avec emoji, concrètes et motivantes en français. Signe tes recommandations "— APEX".`}]})
      });
  ...
  return <div className="afd" style={{background:"#080808",minHeight:"100vh",color:"#ebebeb",fontFamily:ff,maxWidth:440,margin:"0 auto",paddingBottom:96,position:"relative"}}>
    ...
    {celebrate&&<div className="abc" style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:440,zIndex:600,background:`linear-gradient(135deg,${acc},${theme.dark})`,padding:"16px 20px",textAlign:"center",fontSize:14,fontWeight:700,color:"#000",boxShadow:"0 8px 32px rgba(0,0,0,.5)"}}>🎉 Programme 100% complété — Champion !</div>}
    ...
    <div style={{display:"flex",background:"#090909",borderTop:"1px solid #0f0f0f",borderBottom:"1px solid #0f0f0f",marginBottom:14}}>
      {[["today","📅","Auj."],["chain","🔥","Chaîne"],["week","📊","Stats"],["ai","🤖","IA"],["settings","⚙️","Compte"]].map(([id,ico,lbl])=>(
    ...
    <div style={{padding:"0 16px"}}>
      ...
      {tab==="today"&&<div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={resetToday} className="bg" style={{background:"transparent",border:"1px solid #1e1e1e",borderRadius:20,padding:"5px 13px",fontSize:11,color:"#444",cursor:"pointer",fontFamily:ff}}>🔄 Reset</button>
        </div>
        ...
        {todayT.map((t,i)=>{
          ...
          return <div key={t.id} className="tr aup" style={{background:"#0d0d0d",border:`1px solid ${done?"#1e3a1e":"#141414"}`,borderLeft:`3.5px solid ${CAT_COLORS[t.category]||acc}`,borderRadius:15,padding:"12px 8px 12px 12px",display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginBottom:8,opacity:done?0.4:1,animationDelay:`${i*0.03}s`}} onClick={()=>toggleTask(t.id)}>
            ...
            <button onClick={e=>{e.stopPropagation();setPomo(t);}} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:13,padding:"2px 4px",color:"#333",flexShrink:0}} title="Pomodoro">⏱️</button>
      ...
      {tab==="chain"&&<div className="afd" style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <Card>
          ...
          <div style={{fontSize:10,color:"#2a2a2a",textAlign:"center",marginTop:4}}>Basé sur les 14 derniers jours</div>
        </Card>
      </div>}
      ...
      {tab==="ai"&&<div className="afd" style={{display:"flex",flexDirection:"column",gap:14}}>
        <Card>
          ...
          <div style={{fontSize:12,color:"#555",lineHeight:1.9,marginBottom:16}}>APEX analyse ton programme ({tasks.length} tâches · Niv.{lv.level} · {xp} XP) via la neuroscience cognitive et la data science comportementale.</div>
          <button className="bm" onClick={getAI} disabled={aiLoad} style={{width:"100%",background:acc,color:"#000",border:"none",borderRadius:13,padding:14,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:ff,opacity:aiLoad?0.6:1}}>
            <span className={aiLoad?"ash":""}>{aiLoad?"⏳ Analyse en cours...":"🚀 Analyser mon programme"}</span>
          </button>
        </Card>
      ...
      {tab==="settings"&&<div className="afd" style={{display:"flex",flexDirection:"column",gap:14}}>
        <Card>
          ...
          <Div/>
          <div style={{display:"flex",justifyContent:"space-around"}}>
            {[["Niv."+lv.level,"Niveau",acc],[xp+"XP","Total","#CE93D8"],[tasks.length,"Tâches","#66BB6A"]].map(([v,l,c])=><div key={l} style={{textAlign:"center"}}><div style={{fontSize:18,fontWeight:800,color:c}}>{v}</div><div style={{fontSize:10,color:"#3a3a3a"}}>{l}</div></div>)}
          </div>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <button className="bm" onClick={enNotif} style={{width:"100%",background:notifOn?`${acc}22`:"var(--acc)",color:notifOn?acc:"#000",border:notifOn?`1px solid ${acc}`:"none",borderRadius:13,padding:13,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:ff}}>
            {notifOn?"✅ Rappels activés":"🔔 Activer les rappels"}
          </button>
        </Card>
        <Card>
          <ST c={acc}>⚙️ Gérer mon programme</ST>
          <div style={{fontSize:12,color:"#4a4a4a",lineHeight:1.8,marginBottom:14}}>Dans Aujourd'hui : ✏️ modifier · ⏱️ Pomodoro · <span style={{color:acc}}>+</span> ajouter.</div>
        ...
        <Card>
          ...
          <button onClick={()=>setConfirm({msg:"Réinitialiser la progression d'aujourd'hui ?",action:resetToday})} className="bg" style={{width:"100%",background:"transparent",border:"1px solid #0d2a0d",borderRadius:13,padding:13,fontSize:13,color:"#4caf50",cursor:"pointer",fontFamily:ff,marginBottom:10}}>🔄 Réinitialiser aujourd'hui</button>
          <button onClick={()=>setConfirm({msg:"⚠️ Effacer TOUTE la progression ? Action irréversible.",action:resetAll})} className="bg" style={{width:"100%",background:"transparent",border:"1px solid #2a0d0d",borderRadius:13,padding:13,fontSize:13,color:"#ef5350",cursor:"pointer",fontFamily:ff}}>⚠️ Effacer toute la progression</button>
        </Card>
        <Card>
          <ST c={acc}>📱 Installer l'app</ST>
          {[["Chrome Android","Menu ⋮ → Ajouter à l'écran d'accueil","#4FC3F7"],["Vercel","npm create vite → Colle le code → vercel --prod","#66BB6A"],["Play Store","React Native + Expo → Google Play (25$)","#CE93D8"]].map(([t,d,c])=><div key={t} style={{background:"#0a0a0a",borderRadius:12,padding:"11px 13px",marginBottom:8,borderLeft:`2.5px solid ${c}`}}><div style={{fontSize:11,fontWeight:700,color:"#888",marginBottom:4}}>{t}</div><div style={{fontSize:11,color:"#444",lineHeight:1.7}}>{d}</div></div>)}
        </Card>
    ...
    {editModal&&<EditModal task={editModal} onSave={saveTask} onDelete={(id)=>setConfirm({msg:"Supprimer cette tâche définitivement ?",action:()=>delTask(id)})} onClose={()=>setEditModal(null)}/>}
